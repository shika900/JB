///////////////////////  (Original Webkit and Kernel Exploits By Sleirsgoevy & ChendoChap)
// MOD FOR Shika
...
function CalcTime(dur){hrs=Math.floor(dur/1000/60/60);min=Math.floor(dur/1000/60-hrs*60);sec=Math.floor(dur/1000-min*60);mil=dur.toString().slice(-3);if (min!=0){ShowDuration=" - Webkit Vulnerado En : "+min+" minuto"+(min==1?"":"s")+", "+sec+" segundo"+(sec==1?"":"s");}else {ShowDuration=" - Shika Arabic: "+sec+" segundo"+(sec==1?"":"s");}}
...
function kernelExploit() {
 ...
 var trigger_spray = function () {
  ...
  chain.run(); 
  alert("دخل الفلاشة في الجهاز وبعدين دوس موافق بعد لما تظهر الرسالة علي الشمال");
